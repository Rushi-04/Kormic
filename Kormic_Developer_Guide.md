...
* **Artifact Binding & Vendor Squatting Control:** When registering a BAIN, the system mathematically verifies a vendor's signature over the code's true digest (`artifact_digest`). Furthermore, the system implements a strict, verified **Vendor Enrollment Process**: a vendor's public key must be securely enrolled in the replicated `RecordStore` (via `enroll_vendor`) *before* they are allowed to issue BAINs. The `AgentManager` checks this registry to prevent attackers from squatting on a vendor's namespace using self-signed keys. *Note: Until the Trusted Bot Programme introduces automated governance, vendor enrollment remains a carefully controlled, manual-review process to ensure keys genuinely belong to the named entities. The `enrolled_vendors` registry is persisted at the storage layer to ensure multi-replica consensus once distributed.*
