...
# Type must be STU (student), UNI (university), CMP (company), BLD (build), or DPL (deployment)
# Realid_ref must be a 64-char hex string (SHA-256 hash representation)
IDENTITY_REGEX = re.compile(
    r"^KMC\.(STU|UNI|CMP|BLD|DPL)\.([a-zA-Z0-9_-]+)\.([a-zA-Z0-9_\.-]+)\.([a-fA-F0-9]{64})$"
)

@dataclass(frozen=True)
class Identity:
    agent_type: str        # e.g., 'STU' | 'UNI' | 'CMP' | 'BLD' | 'DPL'
    entity_ref: str        # e.g., 'priya7f3a'
    instance: str          # e.g., '0001' or '2.1.0'
    ...
    def validate(self) -> None:
        ...
        if self.agent_type not in {"STU", "UNI", "CMP", "BLD", "DPL"}:
            raise IdentityError(f"Invalid agent type: '{self.agent_type}'. Must be STU, UNI, CMP, BLD, or DPL.")
        ...
        if not re.match(r"^[a-zA-Z0-9_\.-]+$", self.instance):
            raise IdentityError(f"Invalid instance identifier: '{self.instance}'. Must be alphanumeric/dots/hyphens/underscores.")
